...
- (id) initWithURL: (NSString *) url andRequestBody: (NSDictionary *) body andStatus: (NSUInteger *) status;
{
...
- (BOOL) executeGet;
{
     ...
     parameters: nil
      ...
      withBlock: ^(LRRestyResponse *response) {
            if (response.status == [self expectedStatus]) {
                ...
                self._responseStatus = response.status;
    ...
    if ([self responseStatus] != [self expectedStatus]) {
        return FALSE;
    } else {
        return TRUE;
    }
